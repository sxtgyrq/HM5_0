...
namespace TestCore
{
    public class AddrLevelSave
    {
        ...
        public void Setup()
        {
            ...
            AddrLevelSave.startTime = DateTime.Now;

            AddrLevelSave.boundary = new Geometry.Boundary();
            boundary.load(true);

            AddrLevelSave.dt = new Data();
            AddrLevelSave.dt.LoadRoad(AddrLevelSave.boundary, true);
            //Program.dt.LoadModel();
            //Program.dt.LoadCrossBackground();

            //Program.rm = new RoomMainF.RoomMain();

            rm = new HouseManager4_0.RoomMainF.RoomMain(AddrLevelSave.dt);

            AddrLevelSave.npcKey = rm.NPCM.AddNpcPlayer(2, rm, dt);

            rm.AddPlayer(new CommonClass.PlayerAdd_V2()
            {
                c = "PlayerAdd_V2",
                Key = AddrLevelSave.playerKey
            },
            rm, dt
            );
