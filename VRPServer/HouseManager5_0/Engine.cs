            ...
            else
            {
                ...
                if (player.Group.groupNumber >= 2)
                {
                    if (reduceValue >= 500)
                    {
                        ...
                        {
                            var msg = $"在去往{Program.dt.GetFpByIndex(player.getCar().targetFpIndex).FastenPositionName}的路上，【{player.PlayerName}】损失了{CommonClass.F.LongToDecimalString(reduceValue)}";
                            foreach (var item in player.Group._PlayerInGroup)
                            {
